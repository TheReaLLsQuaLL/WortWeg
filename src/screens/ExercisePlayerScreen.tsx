...
export function ExercisePlayerScreen({
  ...
  const finishLesson = async (finishedAttempts: ExerciseAttempt[]) => {
    ...
    await onUpdateState((state) => {
      ...
      const correctAnswers = finishedAttempts.filter(
        (attempt) => attempt.result.correct,
      ).length;
      const xpEarned = finishedAttempts.reduce(
      ...
      return awardXpForStudy({
        ...
        reviewCards: createReviewCardsFromLesson(lesson, state.reviewCards),
